...
void Problem::initialize_boundary(Config* config,
  ...
  for (int index = 0; index < config->num_boundary; index++) {
    ...
    ASSERT1("Problem::initialize_boundary", "Boundary type %s not recognized",
...
Boundary* Problem::create_boundary_(std::string type, int index, Config* config,
  ...
  if (type == "inflow") {
    ...
    if (!(param_type == parameter_list || param_type == parameter_float ||
          param_type == parameter_float_expr)) {
      ERROR2("Problem::create_boundary_()",
             "Parameter %s is of incorrect type %d", param_str.c_str(),
             param_type);
    }
...
Initial* Problem::create_initial_(std::string type, int index, Config* config,
  ...
  if (type == "file") {
    initial = new InitialFile(parameters, config->initial_cycle,
                              config->initial_time);
    ;
  } else if (type == "value") {
...
Method* Problem::create_method_(std::string name, int index_method,
  ...
  if (name == "trace") {
    ...
    method = new MethodNull(config->method_null_dt);
    ...
    method = new MethodCheckpoint(config->method_path_name[index_method]);
    ...
    method = new MethodOutput(factory, config->method_file_name[index_method],
    ...
    method = new MethodRefresh(config->method_field_list[index_method],
                               config->method_particle_list[index_method],
                               config->method_ghost_depth[index_method],
                               config->method_min_face_rank[index_method],
                               config->method_all_fields[index_method],
...
/// @param factory        Factory object for creating Io objects of correct type
{
  ...
  if (name == "image") {
    ...
    output = new OutputImage(
        ...
        image_size, image_reduce_type, image_mesh_color,
