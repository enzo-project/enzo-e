...
namespace cello {
  ...
  int get_type_enum_(const std::type_info &tinfo, bool unknown_on_fail) noexcept
  {
    if (tinfo == typeid(float)){
      return type_single;
    } else if (tinfo == typeid(double)){
      return type_double;
    } else if ((tinfo == typeid(long double)) && (sizeof(long double)==10)){
      return type_extended80;
    } else if ((tinfo == typeid(long double)) && (sizeof(long double)==12)){
      return type_extended96;
    } else if ((tinfo == typeid(long double)) && (sizeof(long double)==16)){
    ...
    } else if (tinfo == typeid(std::int8_t)){
      return type_char;
    } else if (tinfo == typeid(std::int16_t)){
      return type_short;
    } else if (tinfo == typeid(std::int32_t)){
      return type_int;
    } else if (tinfo == typeid(std::int64_t)){
      return type_long_long;
#endif
    }
    ...
    if (unknown_on_fail){
      ...
      // note: type_name is probably mangled, (but it is implementation defined)
      std::string type_name = tinfo.name();
      // note: we need to store type_info in a standalone variable, instead of
      //       calling tinfo.name().c_str(), to avoid lifetime issues...
      ERROR1("get_type_enum_",
             "Can't map the %s type to a type enum.",
             type_name.c_str());
    }
  }
