...
public: // interface
  ...
  Control(std::string curr_name,
	  ...
	  CkCallback next_entry,
	  void (CommBlock::*next_func)()
	  ) throw() 
    : phase_(phase),
      sync_(sync),
  ...
  /// Return the next entry method associated with this Control object, if any
  CkCallback next_entry() const
  { return next_entry_; }
