...
int DataMsgRefresh::id_count = -1;
long DataMsgRefresh::counter = 0;

DataMsgRefresh::~DataMsgRefresh()
{
  // if (field_face_) {
  //   delete field_face_;
  //   field_face_ = 0;
  // }
  // Note: don't delete field_data_
  --counter;
  if (particle_data_) {
...
void * DataMsgRefresh::pack (DataMsgRefresh * msg)
{

#ifdef DEBUG_DATA
  CkPrintf ("DEBUG %p DataMsgRefresh::pack()\n",msg);
#endif
...
DataMsgRefresh * DataMsgRefresh::unpack(void * buffer)
{
  ...
  DataMsgRefresh * msg = (DataMsgRefresh *) CkAllocBuffer (buffer,sizeof(DataMsgRefresh));
  msg = new ((void*)msg) DataMsgRefresh;
#ifdef DEBUG_DATA
  CkPrintf ("DEBUG %p DataMsgRefresh::unpack()\n",msg);
#endif
...
void DataMsgRefresh::update (Data * data)
{
#ifdef DEBUG_DATA
  CkPrintf ("DEBUG %p DataMsgRefresh::update()\n",this);
#endif
  Simulation * simulation = proxy_simulation.ckLocalBranch();
