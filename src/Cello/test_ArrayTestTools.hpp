...
///
/// These functions primarily assist with tests involving CelloArrays

//----------------------------------------------------------------------

/// fill an existing CelloArray with values that start at ``start`` and are
/// incremented by ``step``
template<typename T>
...
/// allocate a new CelloArray and fill it with values that start from ``start``
...
template<typename T>
void assert_allequal3D_(const CelloArray<T,3>& a, const CelloArray<T,3>& b,
