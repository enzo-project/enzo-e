...
std::string Input::expand_file_name () const throw()
{
  ...
  for (size_t i=0; i<file_args_.size(); i++) {
    ...
    if (file_arg == "cycle") {
      ...
      sprintf (buffer_next,buffer_curr, count_input_);
...
double Input::update_timestep (double time, double dt) const
{
...
{
  WARNING("Input::write_simulation()",
	  "This function should not be called");
}

//----------------------------------------------------------------------

void Input::write_hierarchy
