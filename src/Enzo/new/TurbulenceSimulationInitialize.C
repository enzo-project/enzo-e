			       ...
			       HierarchyEntry &TopGrid, TopGridData &MetaData)
{
  ...
  */
  ...
  if (RandomForcing)
    if (RandomForcingMachNumber <= 0.0 && RandomForcingEdot <= 0.0) {
      if (MyProcessorNumber == ROOT_PROCESSOR)
  ...
  /* If RandomForcingEdot (i.e. the energy injection rate) is not set 
     in the parameter file, get it from [MacLow1999] formula. 
     Note: the formula is calibrated for generic forcing fields; 
     coefficient 0.81 can potentially be inappropriate for a purely 
     solenoidal forcing; also our Gamma is not quite 1.0. */
