                          ...
                          int   TotalRefinement)
{
  ...
  */
  ...
  if (RandomForcing && ReadData == TRUE)
  ...
  if (TurbulenceSimulationDensityName != NULL && ReadData == TRUE)
  ...
  if (TurbulenceSimulationTotalEnergyName != NULL && ReadData == TRUE)
  ...
  if (TurbulenceSimulationVelocityNames[0] != NULL && ReadData == TRUE)
    for (dim = 0; dim < GridRank; dim++)
  ...
  /* 3D => If data files for velocities are missing, compute the velocities. */
 
  if (TurbulenceSimulationVelocityNames[0] == NULL && ReadData == TRUE && GridRank == 3)
    if (this->ComputeRandomForcingFields(0) == FAIL) {
      fprintf(stderr, "Error computing random force. \n");
      return FAIL;
    }
 
  /* 2D => If data files for velocities are missing, set the velocities to zero. */

  if (TurbulenceSimulationVelocityNames[0] == NULL && ReadData == TRUE && GridRank == 2)
    for (dim = 0; dim < GridRank; dim++)
      for (i = 0; i < size; i++)
  ...
  if (RandomForcing && TurbulenceSimulationVelocityNames[0] != NULL) {
  ...
  /* In 3D => copy random forcing fields from initial velocities. */
 
    if (ReadData == TRUE && GridRank == 3)
      for (dim = 0; dim < GridRank; dim++)
	for (i = 0; i < size; i++)
	  RandomForcingField[dim][i] = BaryonField[vel+dim][i];

  /* In 2D => compute random forcing fields using mode=1. */

    if (ReadData == TRUE && GridRank == 2)
      if (this->ComputeRandomForcingFields(1) == FAIL) {
	fprintf(stderr, "Error computing random force. \n");
	return FAIL;
      }
