				      ...
				      float * pTopGridTimeStep)
{
  ...
  *pTopGridTimeStep = dt;
  ...
  if (RandomForcing < 3) { // WS: stochastic force (> 2) is already normalised
  ...
  if (debug) printf("RandomForcingGlobals: E %"GSYM"  E_k %"GSYM" E_c %"GSYM" M_v %"GSYM" px %"GSYM" py %"GSYM" pz %"GSYM" norm %"GSYM" \n",
		    (0.5*GlobVal[4]+GlobVal[13])/numberOfGridZones, // mean total energy density (isothermal only)
		    0.5*GlobVal[4]/numberOfGridZones,   // mean kinetic energy density
		    GlobVal[13]/numberOfGridZones,      // mean compressive energy density (isothermal fluids only)
		    //		    sqrt(GlobVal[2]/numberOfGridZones),
                    sqrt(GlobVal[3]/numberOfGridZones), // volume-weighted Mach number
  ...
  if (MetaData->CycleSkipGlobalDataDump != 0)
	...
	ERROR_MESSAGE;
      fprintf( Fptr, "%"ISYM" %9.6"FSYM" %9.6"FSYM" %9.6"FSYM" %9.6"FSYM" %9.6"FSYM" %9.6"FSYM" %9.6"FSYM" %9.6"FSYM" %9.6"FSYM" %9.6"FSYM" %10.5"FSYM"\n",
	       ...
	       GlobVal[13]/numberOfGridZones,       // compressive energy
