...
void EnzoMethodPmDeposit::compute(Block *block) throw() {
  ...
  if (block->is_leaf()) {
    ...
    for (int ipt = 0; ipt < num_mass; ipt++) {
      const int it =
          particle.type_index(particle_groups->item("has_mass", ipt));
        ...
        if (rank == 1) {
          ...
          for (int ip = 0; ip < np; ip++) {
            ...
            de_p[ix0 + mx * (iy0 + my * iz0)] +=
                pdens[ip] * dens * x0 * y0 * z0;
            de_p[ix1 + mx * (iy0 + my * iz0)] +=
                pdens[ip] * dens * x1 * y0 * z0;
            de_p[ix0 + mx * (iy1 + my * iz0)] +=
                pdens[ip] * dens * x0 * y1 * z0;
            de_p[ix1 + mx * (iy1 + my * iz0)] +=
                pdens[ip] * dens * x1 * y1 * z0;
            de_p[ix0 + mx * (iy0 + my * iz1)] +=
                pdens[ip] * dens * x0 * y0 * z1;
            de_p[ix1 + mx * (iy0 + my * iz1)] +=
                pdens[ip] * dens * x1 * y0 * z1;
            de_p[ix0 + mx * (iy1 + my * iz1)] +=
                pdens[ip] * dens * x0 * y1 * z1;
            de_p[ix1 + mx * (iy1 + my * iz1)] +=
                pdens[ip] * dens * x1 * y1 * z1;
    ...
    if (rank >= 2)
      for (int i = 0; i < m; i++) vy[i] = vyf[i];
    else
      for (int i = 0; i < m; i++) vy[i] = 0.0;

    if (rank >= 3)
      for (int i = 0; i < m; i++) vz[i] = vzf[i];
    else
      for (int i = 0; i < m; i++) vz[i] = 0.0;
