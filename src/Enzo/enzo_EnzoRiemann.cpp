...
(std::vector<std::string> integrable_groups, std::string solver)
{
  ...
  if (formatted == std::string("hll")){
    out = new EnzoRiemannHLLMHD(integrable_groups);
  } else if (formatted == std::string("hlle")){
    out = new EnzoRiemannHLLEMHD(integrable_groups);
  } else if (formatted == std::string("hllc")){
    out = new EnzoRiemannHLLC(integrable_groups);
  } else if (formatted == std::string("hlld")){
    // could possibly check that MHD fields are included
    out = new EnzoRiemannHLLD(integrable_groups);
  } else {
