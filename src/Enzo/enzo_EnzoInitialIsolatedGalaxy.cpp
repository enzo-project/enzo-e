...
EnzoInitialIsolatedGalaxy::EnzoInitialIsolatedGalaxy(
  ...
  this->scale_height_ =
      config->initial_IG_scale_height * cello::kpc_cm / enzo_units->length();
  this->scale_length_ =
      config->initial_IG_scale_length * cello::kpc_cm / enzo_units->length();
  this->disk_mass_ =
      config->initial_IG_disk_mass * cello::mass_solar / enzo_units->mass();
  this->gas_fraction_ = config->initial_IG_gas_fraction;
  this->disk_temperature_ =
      config->initial_IG_disk_temperature / enzo_units->temperature();
  this->disk_metal_fraction_ = config->initial_IG_disk_metal_fraction;
  this->gas_halo_mass_ =
      config->initial_IG_gas_halo_mass * cello::mass_solar / enzo_units->mass();
  this->gas_halo_temperature_ =
      config->initial_IG_gas_halo_temperature / enzo_units->temperature();
  this->gas_halo_metal_fraction_ = config->initial_IG_gas_halo_metal_fraction;
  this->gas_halo_density_ =
      config->initial_IG_gas_halo_density / enzo_units->density();
  this->gas_halo_radius_ =
      config->initial_IG_gas_halo_radius * cello::kpc_cm / enzo_units->length();
  ...
  if ((this->gas_halo_density_ == 0.0) && this->gas_halo_mass_ > 0) {
    if (this->gas_halo_radius_ > 0.0) {
      ...
      this->gas_halo_density_ =
          this->gas_halo_mass_; // In code density  mass / 1**3
    }
  } // AE: Need an error check to make sure these are set correctly

  // gather other parameters not associated with this IC
  this->uniform_density_ =
      config->field_uniform_density / enzo_units->density();
  ...
  //    and assigns lifetimes. This allows for IMMEDIATE initial stellar
  //    feedback in the galaxy
  //
  this->include_recent_SF = config->initial_IG_include_recent_SF;
  this->recent_SF_start =
      config->initial_IG_recent_SF_start; // age of oldest newly formed star
  this->recent_SF_end = config->initial_IG_recent_SF_end;
  this->recent_SF_bin_size =
      config->initial_IG_recent_SF_bin_size; // time resolution
...
void EnzoInitialIsolatedGalaxy::enforce_block(
  ...
  ASSERT("EnzoInitialIsolatedGalaxy", "Block does not exist", block != NULL);
  ...
  enzo_float *temperature = field.is_field("temperature")
                                ? (enzo_float *)field.values("temperature")
                                : NULL;

  if (temperature) {
    EnzoComputeTemperature compute_temperature(
        enzo_config->ppm_density_floor, enzo_config->ppm_temperature_floor,
        enzo_config->ppm_mol_weight, enzo_config->physics_cosmology);
...
void EnzoInitialIsolatedGalaxy::InitializeExponentialGasDistribution(
    Block *block) {
  ...
  enzo_float *p = field.is_field("pressure")
                      ? (enzo_float *)field.values("pressure")
                      : NULL;
  ...
  double halo_gas_energy =
      this->gas_halo_temperature_ / this->mu_ / (this->gamma_ - 1);

  double disk_gas_energy =
      this->disk_temperature_ / this->mu_ / (this->gamma_ - 1);
  ...
  for (int iz = 0; iz < mz; iz++) {
    ...
    for (int iy = 0; iy < my; iy++) {
      ...
      for (int ix = 0; ix < mx; ix++) {
        ...
        double disk_density =
            this->gauss_mass(rho_zero, x / enzo_units->length(),
                             y / enzo_units->length(), z / enzo_units->length(),
                             hx) /
            (hx * hy * hz);

        if ((this->gas_halo_density_ * this->gas_halo_temperature_ >
             disk_density * this->disk_temperature_) &&
          ...
          if (metal)
            metal[i] = this->gas_halo_metal_fraction_ * d[i];

        } else if (radius < this->gas_halo_radius_ *
                                enzo_units->length()) // we are in the disk
        {
          ...
          if (this->analytic_velocity_) {
            //            double rhodm =
            //            enzo_config->method_background_acceleration_DM_density;
            double rcore =
                enzo_config->method_background_acceleration_core_radius;
            double rvir =
                enzo_config->method_background_acceleration_DM_mass_radius;
            ...
            vcirc =
                (std::log(1.0 + conc * rx) - (conc * rx) / (1.0 + conc * rx)) /
          ...
          for (int dim = 0; dim < 3;
               dim++) // AE: do I need a check for not ppm?
          {
          ...
          if (metal)
            metal[i] = this->disk_metal_fraction_ * d[i];
...
void EnzoInitialIsolatedGalaxy::InitializeGasFromParticles(Block *block) {
  ...
  for (ipt = 0; ipt < ntypes_; ipt++) {
    if (particleIcTypes[ipt] == GAS_PARTICLE_FLAG)
      break;
  }
  ...
  for (int ip = 0; ip < np; ip++) {
    ...
    if (!(block->check_position_in_block(
            particleIcPosition[ipt][0][ip], particleIcPosition[ipt][1][ip],
            particleIcPosition[ipt][2][ip],
            true // include ghost zones to set properties of gas in ghosts
                 // correctly
            ))) {
      continue;
    }
    ...
    // add momentum
    for (int dim = 0; dim < 3; dim++) {
      v3[dim][i] = particleIcMass[ipt][ip] * particleIcVelocity[ipt][dim][ip] /
                   (hx * hy * hz);
    }
  ...
  for (int iz = 0; iz < mz; iz++) {
    for (int iy = 0; iy < my; iy++) {
      for (int ix = 0; ix < mx; ix++) {
        ...
        if (iflag[i] == 0)
          continue; // skip cells with no deposited particles
    ...
    for (int iy = 0; iy < my; iy++) {
      for (int ix = 0; ix < mx; ix++) {
        ...
        if (mass_deposit > 0) {
          ...
          for (int dim = 0; dim < 3; dim++) {
            v3[dim][i] = (iflag[xh] * v3[dim][xh] * d[xh] +
                          iflag[xl] * v3[dim][xl] * d[xl] +
                          iflag[yh] * v3[dim][yh] * d[yh] +
                          iflag[yl] * v3[dim][yl] * d[yl] +
                          iflag[zh] * v3[dim][zh] * d[zh] +
                          iflag[zl] * v3[dim][zl] * d[zl]) /
                         mass_deposit;
...
void EnzoInitialIsolatedGalaxy::InitializeParticles(Block *block,
  ...
  for (int ipt = 0; ipt < ntypes_; ipt++) {
    ...
    if (it == GAS_PARTICLE_FLAG)
      continue; // do not make these actual particles
    ...
    for (int i = 0; i < np; i++) {
      ...
      //   - only check for active region (particles in ghost zones belong on
      //   other grids)
...
void EnzoInitialIsolatedGalaxy::ReadParticlesFromFile_(const int &nl,
  ...
  this->ReadParticlesFromFile(nl, particleIcPosition[ipt],
                              particleIcVelocity[ipt], particleIcMass[ipt],
  ...
  if (this->include_recent_SF) {
    ...
    if (particleIcFileNames[ipt] == "disk.dat") {
      ...
      int stars_per_bin = floor(
          (this->recent_SF_SFR * this->recent_SF_bin_size * 1.0E3) /
      ...
      for (int ibin = 0; ibin < num_bins; ibin++) {
        ...
        for (int i = 0; i < stars_per_bin; i++) {
          ...
          particleIcCreationTime[ipt][ip] =
              (this->recent_SF_start +
               0.5 * (ibin + 1) * this->recent_SF_bin_size) *
              time_conv;
        }
      }
    }
...
void EnzoInitialIsolatedGalaxy::ReadParticlesFromFile(
    const int &nl, enzo_float **position, enzo_float **velocity,
  ...
  ASSERT("EnzoInitialIsolatedGalaxy", "ParticleFile not found",
         inFile.is_open());
  ...
  while (inFile >> position[0][i] >> position[1][i] >> position[2][i] >>
         velocity[0][i] >> velocity[1][i] >> velocity[2][i] >> mass[i]) {
    ASSERT("EnzoInitialIsolatedGalaxy", "Too many lines in particle file",
           i < nl);
    // positions are in pc
    // velocities in km / s
    // mass in Msun
    ...
    for (int dim = 0; dim < cello::rank(); dim++) {
      ...
      position[dim][i] = position[dim][i] * lu / enzo_units->length() +
                         this->center_position_[dim];
      velocity[dim][i] = velocity[dim][i] * 1000.0 / enzo_units->velocity();
    }
...
void EnzoInitialIsolatedGalaxy::ReadInVcircData(void) {
  ...
  if (this->use_gas_particles_ || this->analytic_velocity_)
    return;
  ...
  ASSERT("EnzoInitialIsolatedGalaxy", "Circular velocity file failed to open",
         inFile.is_open());
...
double EnzoInitialIsolatedGalaxy::InterpolateVcircTable(double radius) {
  ...
  if (i == 0) {
    vcirc = (this->vcirc_velocity[i]) * (radius - this->vcirc_radius[0]) /
            this->vcirc_radius[0];
  } else {
    ASSERT("EnzoInitialIsolatedGalaxy",
...
double EnzoInitialIsolatedGalaxy::gauss_mass(const double rho_zero,
                                             const double xpos,
                                             const double ypos,
                                             const double zpos,
                                             const double dx) {
  ...
  double evaluation_points[5] = {-0.90617985, -0.53846931, 0.0, 0.53846931,
                                 0.90617985};
  double weights[5] = {0.23692689, 0.47862867, 0.56888889, 0.47862867,
                       0.23692689};
  double x_result[5];
  double y_result[5];
  double r, z;
  ...
  for (int i = 0; i < 5; i++) {
    ...
    for (int j = 0; j < 5; j++) {
      ...
      for (int k = 0; k < 5; k++) {
        ...
        y_result[j] += dx / 2.0 * weights[k] * rho_zero *
