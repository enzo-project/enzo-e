...
EnzoInitialFeedbackTest::EnzoInitialFeedbackTest(
  ...
  if (config->initial_feedback_test_from_file){
    ...
    ASSERT("EnzoInitialFeedbackTest",
           "initial_feedback_stars.in failed to open", inFile.is_open());

    int i = 0;
    while (inFile >> this->mass[i] >> this->position[0][i] >>
...
void EnzoInitialFeedbackTest::enforce_block(
  ...
  for (int iz = 0; iz < ngz; iz++) {
    for (int iy = 0; iy < ngy; iy++) {
      for (int ix = 0; ix < ngx; ix++) {
        ...
        d[i] =
            enzo_config->initial_feedback_test_density / enzo_units->density();

        for (int dim = 0; dim < 3; dim++)
          v3[dim][i] = 0.0;

        ge[i] = enzo_config->initial_feedback_test_temperature /
                enzo_config->ppm_mol_weight / enzo_units->temperature() /
  ...
  if (enzo_block->level() != 0)
    return; // don't do particles below root grid
  ...
  int ia_to = particle.has_attribute(it, "creation_time")
                  ? particle.attribute_index(it, "creation_time")
                  : -1;

  int ia_l = particle.has_attribute(it, "lifetime")
                 ? particle.attribute_index(it, "lifetime")
                 : -1;

  int ia_metal = particle.has_attribute(it, "metal_fraction")
                     ? particle.attribute_index(it, "metal_fraction")
                     : -1;
  ...
  for (int i = 0; i < this->num_particles; i++) {
    mask[i] = block->check_position_in_block(
        this->position[0][i], this->position[1][i], this->position[2][i]);
    if (mask[i])
      num_local_particles++;
  }
  ...
  for (int i = 0; i < this->num_particles; i++) {
    if (mask[i]) {
      ...
      CkPrintf("EnzoInitialFeedbackTest (%s): Mass MyPe id_counter istatic id "
               "%f %d %d %d %d\n",
               enzo_block->name().c_str(), this->mass[i], cello::index_static(),
               ParticleData::id_counter[cello::index_static()], CkNumPes(),
               CkMyPe() + (ParticleData::id_counter[cello::index_static()]) *
                              CkNumPes());
#endif
      id[ipp] = CkMyPe() + (ParticleData::id_counter[cello::index_static()]++) *
                               CkNumPes();
      ...
      pform[ipp] = 1.0E-10 * cello::yr_s /
                   enzo_units->time(); // really just needs to be non-zero
