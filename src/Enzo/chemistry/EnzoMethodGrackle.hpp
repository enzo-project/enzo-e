...
public:  // interface
  ...
  EnzoMethodGrackle(CkMigrateMessage* m)
      : Method(m), grackle_facade_(m), use_cooling_timestep_(false) {}
  ...
  virtual std::string name() throw() { return "grackle"; }
  ...
  void setup_grackle_fields(const EnzoFieldAdaptor& fadaptor,
    ...
    grackle_facade_.setup_grackle_fields(fadaptor, grackle_fields, stale_depth,
                                         omit_cell_width);
  }
  ...
  void update_grackle_density_fields(
      Block* block, grackle_field_data* grackle_fields = nullptr) const throw();
  ...
  void calculate_cooling_time(
      const EnzoFieldAdaptor& fadaptor, enzo_float* ct, int stale_depth = 0,
      grackle_field_data* grackle_fields = nullptr) const throw() {
    grackle_facade_.compute_property(fadaptor,
                                     GracklePropertyEnum::cooling_time, ct,
                                     stale_depth, grackle_fields);
  }

  void calculate_pressure(const EnzoFieldAdaptor& fadaptor,
                          enzo_float* pressure, int stale_depth = 0,
                          grackle_field_data* grackle_fields = nullptr) const
      throw() {
