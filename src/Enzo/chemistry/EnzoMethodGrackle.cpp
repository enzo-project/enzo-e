...
EnzoMethodGrackle::EnzoMethodGrackle(
    ParameterGroup p, const double physics_cosmology_initial_redshift,
    const double time)
    : Method(),
...
void EnzoMethodGrackle::update_grackle_density_fields(
    Block* block, grackle_field_data* grackle_fields) const throw() {
#ifndef CONFIG_USE_GRACKLE
  ...
  const enzo_float metal_factor = fluid_floors.has_metal_mass_frac_floor()
                                      ? fluid_floors.metal_mass_frac()
                                      : (enzo_float)tiny_number;
      ...
      for (int ix = 0; ix < ngx; ix++) {
        ...
        if (primordial_chemistry > 0) {
          grackle_fields->HI_density[i] =
              grackle_fields->density[i] * HydrogenFractionByMass;
          grackle_fields->HII_density[i] =
              grackle_fields->density[i] * tiny_number;
          grackle_fields->HeI_density[i] =
              grackle_fields->density[i] * (1.0 - HydrogenFractionByMass);
        ...
        if (primordial_chemistry > 1) {
          grackle_fields->HM_density[i] =
              grackle_fields->density[i] * tiny_number;
          grackle_fields->H2I_density[i] =
              grackle_fields->density[i] * tiny_number;
          grackle_fields->H2II_density[i] =
              grackle_fields->density[i] * tiny_number;
        }

        if (primordial_chemistry > 2) {
          grackle_fields->DI_density[i] =
              grackle_fields->density[i] * DeuteriumToHydrogenRatio;
          grackle_fields->DII_density[i] =
              grackle_fields->density[i] * tiny_number;
          grackle_fields->HDI_density[i] =
              grackle_fields->density[i] * tiny_number;
        }

        if (metal_cooling == 1) {
          grackle_fields->metal_density[i] =
              grackle_fields->density[i] * metal_factor;
        }
      }
    }
  }
...
double EnzoMethodGrackle::timestep(Block* block) throw() {
  ...
  if (use_cooling_timestep_) {
    ...
    enzo_float* cooling_time = field.is_field("cooling_time")
                                   ? (enzo_float*)field.values("cooling_time")
                                   : NULL;
  ...
  for (int iz = 0; iz < ngz; iz++) {
    for (int iy = 0; iy < ngy; iy++) {
      for (int ix = 0; ix < ngx; ix++) {
        int i = INDEX(ix, iy, iz, ngx, ngy);
        metal_density[i] =
            std::max(metal_density[i], metal_mass_frac_floor * density[i]);
      }
    }
  }
  return;
