...
//
//
//    S99 routines are copied over as-is from enzo-dev Fortran
//    (star_maker_ssn.F). Errors there (if any) have been copied over.
//
...
EnzoMethodDistributedFeedback::EnzoMethodDistributedFeedback() : Method() {
  ...
  use_ionization_feedback_ =
      enzo_config->method_feedback_use_ionization_feedback;
...
void EnzoMethodDistributedFeedback::compute_(Block *block) {
  ...
  if (particle.num_particles(it) > 0) {
...
#ifdef DEBUG_FEEDBACK
        CkPrintf("DistributedFeedback (%s) -- Particle %i of %i particles in "
                 "batch %i local at pos (%g, %g, %g) - local = %i\n",
                 enzo_block->name().c_str(), ip, np, ib, px[ipdp], py[ipdp],
                 pz[ipdp], is_local[ipdloc]);
#endif
        /*
           ...
           continue; count++;
                ...
                plifetime[ipdl] *= -1.0;                  // set to negative -
           flag for alreahy gone SNe pmass[ipdm] = pmass[ipdm] -
           total_ejecta_mass_;

        */

        // --------------------------------------- start checking if explosion
        // occurs ---
        ...
        if ((plifetime[ipdl] <= 0.0) ||
          ...
          if (L == 0.0) {
            ERROR("EnzoMethodDistributedFeedback",
                  "Star maker minimum mass is too large for use with "
                  "stochastic model");
          }
          ...
          while (p > L) {
            ...
            float u =
                double(rand()) /
                (double(RAND_MAX)); // float(rand_int%RAND_MAX) / RAND_MAX;
            p *= u;
          }
...
#ifdef DEBUG_FEEDBACK
          CkPrintf("DistributedFeedback (%s) - This particle with m %f and id "
                   "%d has %d supernova \n",
                   enzo_block->name().c_str(), pmass[ipdm], id[ipid],
                   number_of_sn);
#endif
          if (number_of_sn == 0) {
            explosion_flag = 0;
          ...
          for (int kk = 0; kk < number_of_sn; kk++) {
            ...
            double delay_time = p_delay[0] + p_delay[1] * x +
                                p_delay[2] * x * x + p_delay[3] * x * x * x +
                                p_delay[4] * x * x * x * x +
                                p_delay[5] * x * x * x * x * x; // yr
            ...
            double progenitor_mass =
                poly_mass[0] + pmass[1] * td7 + pmass[2] * td7 * td7 +
                poly_mass[3] * td7 * td7 * td7 +
                poly_mass[4] * td7 * td7 * td7 * td7 +
                ...
                poly_mass[9] * td7 * td7 * td7 * td7 * td7 * td7 * td7 * td7 *
                    td7;
            ...
            if ((delay_time > star_age) &&
                (delay_time < star_age + enzo_block->dt)) {
              // AJE: I"m a little confused as to why the above check in
              // original code
              //      was within a time range and not a one-sided check: ---
              //         MUCH LESS CONFUSED NOW: because fixed random number gen
              //         will work
              //                                 if there are unique particle
              //                                 IDs
#ifdef DEBUG_FEEDBACK
              CkPrintf("DistributedFeedback (%18s): %d of %d - delay: %f  "
                       "age+dt: %g  pm: %g \n",
                       enzo_block->name().c_str(), kk + 1, number_of_sn,
                       delay_time, star_age, progenitor_mass);
#endif
              ...
              if (explosion_flag == -1) {
                ...
                ERROR("EnzoMethodDistributedFeedback::compute()",
                      "There was supposed to be no explosions, but this "
                      "particle is flagged");
              } else if (explosion_flag > 0) {
                explosion_flag +=
                    1; // add to the counters - this is not implemented behavior
              } else {
                ERROR("EnzoMethodDistributedFeedback::compute()",
            ...
            //            if ( star_age < delay_time + 0.1 * cello::Myr_s /
            //            enzo_units->time()){
        ...
        //          This ejection rate is correct for the 10^6 solar mass
        //          'cluster'
        //           used to compute these rates with starburst 99.  Reduce to
        //           account for the size of the star particle
        wind_mass = wind_mass *
                    enzo_config->method_star_maker_minimum_star_mass * 1.0E-6;
        wind_mass = wind_mass / cello::yr_s; // in Msun / s
        wind_mass = (wind_mass * enzo_units->time()) *
                    enzo_block->dt; // Msun this timestep

        double tsoon7 =
            soonest_explosion * enzo_units->time() / (1.0E7 * cello::yr_s);
        double wind_energy = 0.0;
        if (time_first_sn_ > 0) {
          // --- for testing ---
          wind_energy = 1.0E48 / (8.0 * cello::mass_solar); // non zero erg
        } else {
          wind_energy = s99_wind_energy(
              td7, tsoon7); // in cm^2/s^2 (i.e. per unit mass in cgs)
        }
        wind_energy =
            wind_energy * wind_mass * cello::mass_solar; // now total E in erg

        star_age = current_time - pcreation[ipdc]; // code units
        td7 = star_age * enzo_units->time() / cello::yr_s /
              1.0E7; // time in 10^7 yr
        ...
        if (explosion_flag > 0) {
          if (time_first_sn_ > 0) { // hack here ! for testing
            ...
            sn_metal_fraction = s99_sn_metallicity(
                td7); // sn metal fraction in fraction (not solar)
          }
          sn_energy = 1.0 * 1.0E51 * explosion_flag; // sn energy in erg
#ifdef DEBUG_FEEDBACK
          CkPrintf("DistributedFeedback (%s) ---- Explosion!! = %g %g %g \n",
                   enzo_block->name().c_str(), sn_mass, sn_energy,
                   sn_metal_fraction);
#endif
        }
        ...
        if (m_eject * cello::mass_solar / enzo_units->mass() > pmass[ipdm]) {
          CkPrintf(
              "WARNING: S99 Distributed Feedback is loosing too much mass\n");
          CkPrintf("setting particle mass to zero, but continuing anyway\n");
          CkPrintf(" %g %g %g %g \n", pmass[ipdm],
                   m_eject * cello::mass_solar / enzo_units->mass(), wind_mass,
                   sn_mass);
        ...
        metal_fraction =
            (pmetal[ipdmf] * wind_mass + sn_metal_fraction * sn_mass) / m_eject;

        // ---------------------------------------- end determining explosion
        // properties ----
        ...
        this->add_ionization_feedback(block, xpos, ypos, zpos, s49_tot,
                                      will_explode);

        // -----

        this->inject_feedback(block, xpos, ypos, zpos, m_eject,
                              (energy / 1.0E51),
                              enzo_config->method_feedback_ke_fraction,
                              metal_fraction, pvx[ipdv], pvy[ipdv], pvz[ipdv]);
        // remove mass - error checking on the std::max is handled above with
        // warning
        pmass[ipdm] = std::max(0.0, pmass[ipdm] - m_eject * cello::mass_solar /
                                                      enzo_units->mass());
        /*
    ...
    if (count > 0) {
      CkPrintf(
          "DistributedFeedback (%s) - Number of feedback particles:   %i \n",
          enzo_block->name().c_str(), count);
    }
#endif

#ifdef DEBUG_FEEDBACK
    CkPrintf("DistributedFeedback (%s) ------------1------------ Number of "
             "local / total particles : %i %i\n",
             enzo_block->name().c_str(), particle.num_local_particles(it),
...
#ifdef DEBUG_FEEDBACK
    CkPrintf("DistributedFeedback (%s) ------------2------------ Number of "
             "local / total particles : %i %i\n",
             enzo_block->name().c_str(), particle.num_local_particles(it),
             particle.num_particles(it));
#endif
...
void EnzoMethodDistributedFeedback::add_ionization_feedback(
    Block *block, double xpos, double ypos, double zpos, const double &s49_tot,
    const int &will_explode) {

  if (!(this->use_ionization_feedback_))
    return;
  ...
  if (shift_cell_center_) {
    ...
    if (((xpos - (stencil_rad_ + 1) * hx) <
         xm) || // note: xm/xp is min/max without including ghost
        ((xpos + (stencil_rad_ + 1) * hx) >
         xp) || //     +1 b/c of CIC inerpolation onto grid
  ...
  double ndens = d[index] * enzo_units->density() /
                 enzo_config->ppm_mol_weight / cello::mass_hydrogen;

  double stromgren_radius = std::pow(
      (3.0 * s49_tot * 1.0E49) / (4.0 * cello::pi * alpha * ndens * ndens),
      1.0 / 3.0);
  double stromgren_volume = (4.0 / 3.0) * cello::pi * stromgren_radius *
                            stromgren_radius * stromgren_radius;
  stromgren_volume /=
      (enzo_units->length() * enzo_units->length() * enzo_units->length());
  double ionized =
      cello::kboltz * 1.0E4 / std::min(0.6, enzo_config->ppm_mol_weight) /
...
#ifdef DEBUG_FEEDBACK
  CkPrintf("DistributedFeedback (%s): This cell should be getting ionized with "
           "radius = %g (pc). volume ratio = %g. will_explode = %i ge = %g "
           "ionized = %g\n",
           enzo_block->name().c_str(), stromgren_radius / cello::pc_cm,
           stromgren_volume * inv_volume, will_explode, ge[index], ionized);
#endif
...
void EnzoMethodDistributedFeedback::inject_feedback(
    Block *block, double xpos, double ypos, double zpos,
  ...
  /*
    ...
    pvx,pvy, and pvz are the local velocity of the injection source. These are
    only needed if ke_fraction > 0. By default these are set to the local gas
    flow
  ...
  */
  ...
  const double energy_units =
      enzo_units->mass() * enzo_units->velocity() * enzo_units->velocity();
#ifdef DEBUG_FEEDBACK
  CkPrintf(
      "Injecting feedback in %i cells at mass (Msun) %g and energy (E51) %g\n",
  ...
  enzo_float *u_local = 0, *v_local = 0, *w_local = 0, *d_local = 0,
             *ge_local = 0, *te_local = 0;
  ...
  if (ke_f < 0) {
    ...
    if (avg_z < 0.01) {
      ...
      t_PDS = 26.50 * pow(E_51, 3.0 / 14.0) * pow(avg_z, -5.0 / 14.0) *
              pow(avg_n, -4.0 / 7.0);
      R_PDS = 18.50 * pow(E_51, 2.0 / 7.0) * pow(avg_z, -1.0 / 7.0) *
              pow(avg_n, -3.0 / 7.0);
    ...
    if (R_PDS > n_resolve * R_resolve) {
      ...
      ke_f = 3.97133E-6 * (avg_d / cello::mass_hydrogen) *
             (1.0 / (R_resolve * R_resolve)) * pow(R_PDS, 7) *
             (1.0 / (t_PDS * t_PDS)) * (1.0 / E_51);
  ...
  for (int k = iz - stencil_rad_; k <= iz + stencil_rad_ + 1; k++) {
    if ((k < 0) || (k >= mz)) {
      loc_index += (stencil_ + 1) * (stencil_ + 1);
      continue;
    }
    for (int j = iy - stencil_rad_; j <= iy + stencil_rad_ + 1; j++) {
      if ((j < 0) || (j >= my)) {
        loc_index += (stencil_ + 1);
        continue;
      }
      for (int i = ix - stencil_rad_; i <= ix + stencil_rad_ + 1; i++) {
        ...
        ke_before[loc_index] =
            0.5 * d[index] *
            (v3[0][index] * v3[0][index] + v3[1][index] * v3[1][index] +
  ...
  if (ke_f > 0) {
    this->convert_momentum(v3[0], v3[1], v3[2], d, pvx, pvy, pvz, mx, my, mz,
                           ix, iy, iz, 1);

    // compute the total mass and energy in the cells before the explosion
    this->sum_mass_energy(v3[0], v3[1], v3[2], d, ge, te, mx, my, mz, ix, iy,
                          iz, sum_mass_init, sum_energy_init, sum_ke_init);
    ...
    this->add_feedback_to_grid(
        u_local, v_local, w_local, d_local, ge_local, te_local, metal_local,
        stencil_ + 1, stencil_ + 1, stencil_ + 1, // mx,my,mz for local grid
        stencil_rad_, stencil_rad_,
        stencil_rad_, // local grid cell center  - should be 1 for 3x3x3 stencil
                      // (0,1,2) - 2 for 5x5x5 stencil (0,1, 2, 3,4)
        dxc, dyc, dzc, density_per_cell, 1.0, 0.0, metal_fraction);
  }
  // momenum injection - compute coefficients for injection
  double mom_per_cell = 0.0;
  if (ke_f > 0) {
    double A = 0.0, B = 0.0, C = 0.0;
    this->compute_coefficients(v3[0], v3[1], v3[2], d, ge, u_local, v_local,
                               w_local, d_local, mx, my, mz, ix, iy, iz, A, B,
                               C);
  ...
  this->add_feedback_to_grid(v3[0], v3[1], v3[2], d, ge, te, metal, mx, my, mz,
                             ix, iy, iz, dxc, dyc, dzc, density_per_cell,
                             mom_per_cell, E_therm, metal_fraction);

  double sum_mass_final, sum_energy_final, sum_ke_final;

  if (ke_f > 0) {
    this->sum_mass_energy(v3[0], v3[1], v3[2], d, ge, te, mx, my, mz, ix, iy,
                          iz, sum_mass_final, sum_energy_final, sum_ke_final);
...
void EnzoMethodDistributedFeedback::convert_momentum(
    ...
    const int &mx, const int &my, const int &mz, const int &ix, const int &iy,
    const int &iz, int idir) {
...
void EnzoMethodDistributedFeedback::sum_mass_energy(
    ...
    enzo_float *ge, enzo_float *te, const int &mx, const int &my, const int &mz,
    const int &ix, const int &iy, const int &iz, double &sum_mass,
    double &sum_energy, double &sum_ke) {
...
void EnzoMethodDistributedFeedback::add_feedback_to_grid(
    enzo_float *px, enzo_float *py, enzo_float *pz, enzo_float *d,
    enzo_float *ge, enzo_float *te, enzo_float *metal, const int &mx,
    const int &my, const int &mz, const int &ix, const int &iy, const int &iz,
        ...
        for (int i1 = i; i1 <= i + 1; i1++) {
          ...
          for (int j1 = j; j1 <= j + 1; j1++) {
            ...
            for (int k1 = k; k1 <= k + 1; k1++) {
              ...
              // cell
              double delta_pu = ((i > 0) ? 1 : (i < 0 ? -1 : 0)) *
                                mom_per_cell * dxc1 * dyc1 * dzc1;
              double delta_pv = ((j > 0) ? 1 : (j < 0 ? -1 : 0)) *
                                mom_per_cell * dxc1 * dyc1 * dzc1;
              double delta_pw = ((k > 0) ? 1 : (k < 0 ? -1 : 0)) *
                                mom_per_cell * dxc1 * dyc1 * dzc1;
              ...
              // CkPrintf("xyz - add_FB_loop: %d %e %e", index, delta_therm,
              // delta_mass);
              ...
              if (metal)
                metal[index] = (metal[index] + delta_mass * metal_fraction);

              // account for multi-species (H,He,etc.) here, along with
              // additional metal species fields if they are present
...
void EnzoMethodDistributedFeedback::compute_coefficients(
    ...
    enzo_float *d_l, const int &mx, const int &my, const int &mz, const int &ix,
    const int &iy, const int &iz, double &A, double &B, double &C) {
