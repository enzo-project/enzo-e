...
EnzoMethodMergeStars::EnzoMethodMergeStars(double merging_radius_cells)
    : Method(), merging_radius_cells_(merging_radius_cells) {
  ...
  ASSERT("EnzoMethodMergeStars::EnzoMethodMergeStars()",
         "EnzoMethodMergeStars requires that we run a 3D problem (Domain: rank "
         "= 3)",
         cello::rank());
...
void EnzoMethodMergeStars::compute_(Block *block) {
  ...
  CkPrintf("The %d particles on Block %s are in %d FoF groups \n",
           num_particles, block->name().c_str(), ngroups);
#endif
  ...
  for (int i = 0; i < ngroups; i++) {
    ...
    CkPrintf("Group %d out of %d on block %s: Group size = %d \n", i + 1,
             ngroups, block->name().c_str(), group_size[i]);
#endif
    ...
    if (group_size[i] > 1) {
      ...
      for (int j = 1; j < group_size[i]; j++) {
        ...
        CkPrintf("Merger in Group %d: \n Particle 1: Mass = %g, "
                 ...
                 "Particle 2: Mass = %g, "
                 "Position = (%g,%g,%g), ID = %ld \n",
                 i + 1, pmass1, px1, py1, pz1, pid1, pmass2, px2, py2, pz2,
                 pid2);
  ...
  // Now we delete non-local particles, i.e. particles with positions
  // out-of-bounds of the block.
...
void EnzoMethodMergeStars::get_particle_coordinates_(
    EnzoBlock *enzo_block, int it, enzo_float *particle_coordinates) {
  Hierarchy *hierarchy = cello::hierarchy();
...
bool EnzoMethodMergeStars::particles_in_neighbouring_blocks_(
    EnzoBlock *enzo_block, enzo_float *particle_coordinates, int **group_lists,
  ...
  for (int j = 0; j < group_size[group_index]; j++) {
    ...
    if (px1 > 0.0 && px1 < 1.0 && py1 > 0.0 && py1 < 1.0 && pz1 > 0.0 &&
        pz1 < 1.0)
      continue;
    ...
    for (int k = j; k < group_size[group_index]; k++) {
      ...
      // In each dimension, check if the coordinate of one of pair is less than
      // 0 and the other greater than 1.
      if (px1 < 0.0 && px2 > 1.0 || px2 < 0.0 && px1 > 1.0 ||
          py1 < 0.0 && py2 > 1.0 || py2 < 0.0 && py1 > 1.0 ||
          pz1 < 0.0 && pz2 > 1.0 || pz2 < 0.0 && pz1 > 1.0) {
        return_val = 0;
