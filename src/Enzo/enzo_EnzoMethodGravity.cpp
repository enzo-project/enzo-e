...
EnzoMethodGravity::EnzoMethodGravity(int index_solver, double grav_const,
                                     int order, bool accumulate,
                                     int index_prolong, double dt_max)
    : Method(),
  ...
  if (accumulate) {
    ...
    ASSERT1("EnzoMethodGravity::EnzoMethodGravity()",
            ...
            name().c_str(), (prolong_name != "enzo"));

    refresh->set_accumulate(true);
    refresh->add_field_src_dst("density_particle",
...
void EnzoMethodGravity::compute(Block* block) throw() {
  ...
  ASSERT("EnzoMethodGravity::compute",
         "modifying density in EnzoMethodGravity?", idensity != id);
...
void EnzoMethodGravity::refresh_potential(EnzoBlock* enzo_block) throw() {
  ...
  enzo_block->refresh_start(ir_exit_,
  ...
  for (int iz = gz; iz < mz - gz; iz++) {
    for (int iy = gy; iy < my - gy; iy++) {
      for (int ix = gx; ix < mx - gx; ix++) {
        ...
        if (rank == 3)
          a_mag_2 =
              a3[0][i] * a3[0][i] + a3[1][i] * a3[1][i] + a3[2][i] * a3[2][i];
#else
        if (rank == 1) a_mag_2 = ax[i] * ax[i];
        if (rank == 2) a_mag_2 = ax[i] * ax[i] + ay[i] * ay[i];
        if (rank == 3) a_mag_2 = ax[i] * ax[i] + ay[i] * ay[i] + az[i] * az[i];
#endif
