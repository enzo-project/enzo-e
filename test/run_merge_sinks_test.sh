...
python $testPrefix/run_merge_sinks_test.py --launch_cmd "charmrun +p4 ++local ./bin/enzo-e" --prec $CELLO_PREC --ics_type stationary > test/merge_sinks_stationary_parallel.test-log 2>&1
ERR_CODE=$?				   
if [ $ERR_CODE -gt 0 ]; then
    ((PARALLEL_FAILED++))
    echo "FAILED"
