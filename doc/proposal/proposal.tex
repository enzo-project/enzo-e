...
mesh refinement (AMR).  
%
Distinguishing characteristics of the software relative to existing
AMR libraries and frameworks include the aggressive pursuit of extreme
scalability with respect to hardware parallelism and software data
structures in its design and implementation.
%
The resulting AMR framework will enable application developers to
write multiphysics applications for simulating phenomena on an
unprecidented range of spacial and temporal scales.
%
The framework will be designed from the ground up to efficiently use
hierarchical parallelism and memory of the largest currently existing
supercomputers, and designed to be adaptable to future HPC platforms
as they evolve through the petascale era and into the exascale.

\ \\ \pargraph{data structure scope summary}
%
AMR will be implemented using a fully distributed generalization of
the octree data structure.  Tree nodes will be associated with
both arrays and particle data, to support Eularian, Lagrangian, and
hybrid methods.

\ \\ \pargraph{parallelization strategy}
%
Parallelization will be primarily data parallel (SPMD), though task
parallelization (MPMD) will also be supported to augment the data
parallelism in cases where data parallelism is insufficient.  A wide
variety of parallelization technologies will be supported from the
start, including both one- and two-sided message passing via the
MPI~\cite{@@@MPI} library, the partitioned global address space
approach via the UPC~\cite{@@@UPC} language), shared memory parallel
programming using OpenMP directives, and the processor virtualization
approach provided by the CHARM++ framework~\cite{@@@CHARM}.  Select
hybrid approaches will also be supported, including MPI with OpenMP,
and MPI with UPC.
...
cabinets, compute nodes, sockets, cores).~\cite{@@@LAN-DLB}.  Advantages will
include the elimination of the scaling problem  to loada reducing the
number of of flexibility in load balancing frequencies and metrics at
individual levels.
%
...
aggressively localizing the individual AMR patch / block tasks;
this will help control scalability issues related to limited
...
The \cello\ Project began as a successor to the parallel AMR
astrophysics and cosmology application \enzo\ \cite{@@@ENZO}.  \enzo\
was conceived in the early 1990's by Michael L.~Norman and Greg Bryan,
and implemented using structured (patch-based) adaptive mesh
refinement (SAMR) \cite{@@@SAMR}.  It incorporated a modified
high-order Piecewise Parabolic Method (PPM) solver \cite{@@@PPM} for
hydrodynamics, and a Particle-Mesh (PM) method \cite{@@@PM} for dark
...
refinement in particular.  Computational load must be well-distributed
across computational components to maintain high parallel efficiency,
and dynamically allocated memory must be well-distributed across
distributed memory nodes to prevent depletion of physical memory.
...
space-filling curve, and partition the tree nodes among processes by
dividing the linearized structure into $N_P$ evenly sized pieces.
This approach works well if the work and memory usage are proportional
to each other, and are roughly equal between tree nodes
but requires $O(P)$ storage 
