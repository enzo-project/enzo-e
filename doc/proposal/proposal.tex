...
The high-level components include \code{Task} for defining parallel
tasks, \code{Distribute} for distributing and load-balancing tasks
across available resources, and \code{Schedule} for scheduling tasks
for execution.

The middle-level components include \code{Simulation} for defining the
simulation, and \code{Method} for user-supplied functions for
...
including \code{Disk} for disk I/O, \code{Memory} for memory
...
The cross-cutting concern components include \code{Parameters} for
reading, storing, and accessing parameters, \code{Monitor} for logging
performance and status information, and \code{Portal} for dynamically
interacting with external utilities or systems.  \code{Performance} is
used for collecting and accessing performance-related data, and
\code{Error} is used for signalling hardware or software faults.
