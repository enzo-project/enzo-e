...
mesh refinement (AMR).  Its main distinction is it will be designed to
allow application developers to write multiphysics applications for
simulating phenomena on an extreme range of spacial and temporal
scales, and that require an extreme amount of computation, memory, and
storage.
%
\ \\ \pargraph{data structure scope summary} Our basic data structure
is a fully parallel distributed generalized octree, with tree nodes
associated with both array and particle data to support both Eularian
and Lagrangian methods.
%
\ \\ \pargraph{parallelization strategy} Parallelization will be
...
\pargraph{load balancing} Dynamic load balancing / task migration will
be hierarchical, both to better adapt to the hierarchical nature of
extreme scale computational platforms (cabinets, nodes, sockets,
cores), and to provide flexibility in load balancing frequencies and
metrics at individual levels.
%
...
aggressively localizing the individual AMR patch / block problems;
...
\begin{verbatim}
 ...
 software resiliency
    resistent to continuous hardware and software failures
       memory failure / local node limits
       compute component (node, socket, core) failures
       network failures
       disk failures
       algorithm failure
\end{verbatim}
...
parallel HDF5
I/O performance
   design: support I/O from subset of nodes , dedicated or shared, to
...
%-----------------------------------------------------------------------
\subsection{High level components} \label{ss:components}
%-----------------------------------------------------------------------
...
%-----------------------------------------------------------------------
\subsection{Data placement} \label{ss:data-placement}
%-----------------------------------------------------------------------
