...
Our framework will be resilient to multiple compute, memory, disk, interconnect, and
software failures.  This will depend on the ability to detect failures, which
may require OS or library support not currently available.
Even if these requirements may not be fully realizable yet, resilience still indicates
properties of the software framework, including virtualization of processors and nodes,
and the ibility to flag components as unreliable.
...
supported, though this is unlikely to scale well due to the magnitued
and rate of change in disparity in performance between disk I/O and
computation, and is sensitive to disk faults.  The software will also
support checkpointing to memory, which is a known
technique~\cite{@@@CHARM}.  This approach scales better, but of course
reduces the amount of available memory for the application.
