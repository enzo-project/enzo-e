...
%
% patch  structured AMR (SAMR)
% block  octree-based AMR
% grid   single block grid
% mesh   entire mesh hierarchy
%
%=======================================================================
...
%=======================================================================
\TITLE{\cello: A New Software Framework for \\ Extreme Adaptive Mesh
...
onset of extreme scalability, in terms of both software data
structures and hardware parallelism.  The design will be
forward-looking, targeting not just the largest existing
supercomputers, but also expected future large-scale HPC platforms as
...
MPI with OpenMP and MPI with UPC.  Dynamic load balancing of parallel
...
ultimately non-scalable, so we will support other alternatives,
...
blocks) and depth (number of AMR levels); using patch-local adaptive
...
supporting variable grid block sizes while still maintaining bounded
...
\samrai\ is written in \cpp, and parallelized using MPI.  Its
high-level design consists of a collection of functionally decomposed
components.  Low-level components include \code{Toolbox} for basic
memory, pointer, and array utilities, and \code{Math Operations} for
basic arithmetic and BLAS-type functions.  AMR hierarchy components
include \code{Hierarchy}, which is used to represent a distributed AMR
data structure, and \code{Mesh}, which contains operators on the
hierarchy, such as remeshing, tagging cells, and load balancing.
Patch data, including arrays, particles, and embedded boundaries, are
represented using the \code{Patchdata} component.  Other components
include support for solvers, algorithms, geometries, and
visualization.
...
methods for solving coupled hyperbolic, elliptic, and parabolic
PDE's.  \chombo\ is quite scalable, and designed to run both
...
components: \code{BoxTools} for set calculus on point sets and unions
of rectangles, \code{AMRTools} for interprocess communication between
AMR levels, \code{AMRTimeDependent} for advancing the solution using
adaptive time steps, \code{AMRElliptic} for the multigrid-based
solution of elliptic AMR problems, and \code{EBTools} for embedded
boundaries.  A sixth component, \code{ParticleTools} for particle
...
blocks that have fixed size, though the grid block size can be set at
compile time.  Grid blocks are represented using a five dimensional
...
supported by the framework, it has been used with the \code{BoomerAMG}
algebraic multigrid solver~\cite{HeYa02}, which is available in the
\code{hypre} scalable linear solver package~\cite{FaJo06}.
...
space-filling curves.  However, as implemented the \alps\ framework
does not meet all of our specific requirements.  For example, it does
not support particles, its octree nodes support individual finite or
spectral elements rather than small grid blocks, and it does not
appear to support adaptive time stepping (due in part to its assigning
an equal number of elements to each core~\cite{BuGh08b}).  Also, we
believe that the efficiency of refinement in localized regions of
refined but uniform resolution can be significantly improved for
octree-based AMR in general.  Nevertheless, due to its impressive
demonstrated scalability, we expect that the \alps\ framework will
...
\gadget-3.0 also supports multithreading in addition to MPI.
...
tasks across available resources, and \code{Schedule} for dynamically
scheduling the tasks for execution.

The middle-level components include \code{Simulation} for defining the
simulation, and \code{Method} for encapsulating user functions for
...
octree-based AMR approach is to allow variable grid sizes per AMR
...
\FIGURETWOCOL{Development Phases}{t:phases}{
