...
%=======================================================================
\TITLE{A Software Framework for \\ Extreme Adaptive Mesh
...
using a linearized Morton ordered space-filling curve.  Solving
elliptic problems is not directly supported, but it has been used with
the \code{BoomerAMG} algebraic multigrid solver~\cite{HeYa02},
...
not support particles, and it does not appear to be applicable to
adaptive time stepping (due in part to distributing equal number of
elements to each core~\cite{BuGh08b}).  Nevertheless, we expect the
\alps\ framework to significantly influence our design decisions.
...
implement the \code{Disk} component of \cello\ using parallel HDF5,
