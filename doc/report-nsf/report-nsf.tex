...
\begin{abstract}
This document describes \cello, a new parallel adaptive mesh refinement
(AMR) software framework currently under development.  The purpose of
\cello\ is to enable researchers to write multi-physics AMR
...
\SUBSUBSECTION{Scalability issues} There are countless issues
...
\SUBSUBSECTION{Approach and solutions} We will address scalability issues
...
%performance and scalability, a built-in spatial data structure that
...
% collective statistical properties still converge.
In \cite{OsNa05} it
...
% 
% Spatial dimensions will include $3$D, but the framework will also
...
% spatial data structure or other optimization be used, such as octrees,
...
% distributed spatial data structure must be used, such as an octree or
...
% field values, and arbitrary logical expressions involving spatial
...
utilities or systems, \code{Performance} for collecting and allowing access
to performance data, and \code{Error} for signalling hardware or
...
\FIGURE{$2$D illustration of patch coalescing: replacing four AMR patches with a single larger one}{f:coalesce}{
...
% coalescing.  Even though other source problems are more
% well-suited for this technique, the resulting quadtree nevertheless
% contains $2.5$ times fewer nodes.
...
\FIGURE{ Coalesced patch refinement using a circle source.  \textbf{Left}: Source circle.  \textbf{Middle}:
  Balanced quadtree with 18517 non-coalesced patches.  \textbf{Right}:
  Balanced quadtree with 789 coalesced patches.  } {f:cosmo}{
\begin{minipage}{\textwidth}
\begin{center}
\begin{minipage}{2.0in}
\includegraphics[width=2.0in]{circle.png}
\end{minipage} \ 
\begin{minipage}{2.0in}
\includegraphics[width=2.0in]{circle-1.png}
\end{minipage} \ 
\begin{minipage}{2.0in}
\includegraphics[width=2.0in]{circle-2.png}
\end{minipage}
\end{center}
\end{minipage}
}
...
\SUBSUBSECTION{Distributed AMR data structure}
%
...
%
% Tasks are encapsulated in the \code{Task} software component. 
...
%be used to define a parallel task.  
%In \cello, a task may be an entire
%patch, or a subblock.  
...
%\code{Schedule} software component.  
...
sustainability.  
%
%And fourth, the framework could be used as a
...
% involving spatial variables \code{x}, \code{y}, and \code{z}, and time
...
including the \code{Monitor} component to display pertinent
performance information to the user, and the \code{Distribute}
...
control repository.  GNU \code{make} or \code{SCons} will be used for compiling
...
%Performance testing will be aided
...
%                 locally override spatial mesh width / time steps
%    Software reviews
...
with each iteration.  
%We expect the initial cycle to require the most
...
%integrated with each other, strictly controlling
%interdependencies.  
...
\SUBSUBSECTION{Quality control} Quality control will be a major ingredient 
...
\SUBSUBSECTION{Education and Outreach Plan} The international 
