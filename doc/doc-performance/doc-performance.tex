...
\devel{Desired features}
 ...
 Even though only ``local'' \code{grid} objects have memory allocated
 for data fields, each MPI process stores both local and remote
 \code{grid}s.  The total memory is thus \[ \numgrids (\numprocs |G|
...
\code{grid} objects.  The total memory is thus 
\[ \numgrids (\numprocs |G| + |F|)\]
...
petascale-level computing, the memory overhead would dominate,
...
is the number of processors, and $\sizegrid$ is the memory required
...
This modification would change the overhead memory term from
$\numgrids \numprocs \sizegrid$ to $\numgrids ((\numprocs-1)
...
%-----------------------------------------------------------------------
  ...
  memory for cubical grid patch is $23^3$, meaning that a grid
  ...
  However, it would be possible to optimize memory use versus hiding
...
%-----------------------------------------------------------------------
  ...
  per shared memory node, which would decrease the memory further by a
  factor equal to the number of processors per node. This could easily
  be implemented with one of the hybrid parallel models described in
  the next section.
