...
The astrophysics and cosmology application ~\cite{OsBr04}\enzo\ was
conceived in the early 1990's by Michael L.~Norman and Greg Bryan, and
implemented using structured adaptive mesh refinement
(SAMR)~\cite{BeCo89}.  It incorporated a modified high-order Piecewise
Parabolic Method (PPM) solver for hydrodynamics~\cite{WoCo84b}, and a
Particle-Mesh (PM) method for dark matter dynamics~\cite{HoEa88}.  So
far in its 15 year lifetime, \enzo\ simulations have led to important
scientific contributions to the fields of astrophysics, cosmology, and
...
used as elements.  Using only the leaves can reduce communication
compared to both the SAMR approach, as well as octree hierarchies in
which all tree nodes are used as elements, since no communication is
required between levels~\cite{BuGh08b}.  The octree is kept balanced
to limit level jumps to at most one level difference, and the
...
\textbf{Version 0.1: Initial Hyperbolic Version.}~The initial phase will support
hyperbolic problems on a single-level ``unigrid'' mesh, parallelized
with \charm.  
%This capability is on the surface simple, but \charm\ is
...
%task scheduling, and dynamic load balancing. 
...
\textbf{Version 0.2: Basic AMR Version.}~The second phase will involve
...
\textbf{Version 0.3: Elliptic Version.}~This phase will involve
...
\textbf{Version 0.4: Advanced AMR Version.}~The next phase will
...
\textbf{Version 0.5: Multi-Parallel Version.}~The fifth phase will
involve adding other parallelization support, including MPI-2
one-sided communication, OpenMP threading, and UPC.  The
parallelization approach will be the same as with \charm, but the
low-level parallel communication and synchronization routines will be
updated.  This will also involve explicitly implementing the task
scheduling and load balancing functionality supported by the
higher-level \charm\ framework.
